...
>
  ...
  <!-- Cost Widget -->
  {#if costSummary}
    <div class="px-4 py-2 border-t border-border" title="Chi phí ước tính theo API, không phải hóa đơn thực tế">
      <div class="flex items-center justify-between text-xs mb-1">
        <span class="text-text-muted">Hôm nay</span>
        <span
      ...
      <div class="flex items-center justify-between text-xs mb-1.5">
        <span class="text-text-muted">Tháng này</span>
      ...
      {#if costSummary.daily_exceeded || costSummary.monthly_exceeded}
        <p class="text-[10px] text-danger mt-1">Đã vượt ngân sách!</p>
      {/if}
    </div>
  {/if}
...
{#if showAbout}
  <div class="fixed inset-0 z-50 flex items-center justify-center">
    <button class="absolute inset-0 bg-black/50" onclick={() => (showAbout = false)} aria-label="Đóng"></button>
    <div class="relative bg-bg-secondary border border-border rounded-2xl shadow-2xl w-96 p-8 text-center z-10">
      <button class="absolute top-3 right-3 p-1 text-text-muted hover:text-text-primary" onclick={() => (showAbout = false)} aria-label="Đóng">
        <XIcon size={16} />
      </button>

      <img src={logoUrl} alt="Harness" class="w-64 h-auto mx-auto mb-4" />
      <h2 class="text-xl font-bold text-text-primary">Harness</h2>
      <p class="text-sm text-text-muted mt-1">Trình quản lý cấu hình AI cho Claude Code</p>
      <p class="text-xs text-text-muted mt-1">Phiên bản {appVersion}</p>
      ...
      <div class="mt-6 pt-4 border-t border-border">
        <p class="text-xs text-text-muted">
          Được xây bởi <span class="text-accent">Quang Tuoi</span>
        </p>
        <p class="text-[10px] text-text-muted mt-1">Giấy phép AGPL-3.0</p>
      </div>
    </div>
  </div>
{/if}
