<script lang="ts">
  ...
  onMount(async () => {
    try {
      ...
      // Trình cập nhật chưa được cấu hình hoặc không có bản mới - bỏ qua
    }
  });
...
{#if updateAvailable && !dismissed}
  <div class="flex items-center justify-between px-4 py-2 bg-accent/10 border-b border-accent/20 shrink-0">
    <div class="flex items-center gap-2 text-xs">
      ...
      <span class="text-text-primary">Harness <strong>{updateVersion}</strong> đã có bản mới</span>
    </div>
