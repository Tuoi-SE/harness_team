...
<div class="bg-bg-secondary border border-border rounded-lg p-4 space-y-4">
  <h3 class="text-sm font-medium text-text-secondary">Quyền truy cập</h3>
  ...
  <div class="flex gap-2">
    ...
    <input
      ...
      placeholder='vd: Bash(npm run *), Edit(/src/**/*.ts), WebFetch(domain:github.com)'
      bind:value={newRule}
  ...
  <!-- Rules Lists -->
  {#each [
    { key: "allow" as const, label: "Cho phép", color: "text-success" },
    { key: "ask" as const, label: "Hỏi", color: "text-warning" },
    { key: "deny" as const, label: "Từ chối", color: "text-danger" },
  ] as section}
