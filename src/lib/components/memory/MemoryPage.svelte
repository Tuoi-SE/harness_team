...
<ConfirmDialog
  ...
  title="Xóa file bộ nhớ"
  message="File bộ nhớ này sẽ bị xoá vĩnh viễn."
  ...
  <div class="w-56 shrink-0 border-r border-border flex flex-col bg-bg-secondary">
    ...
    <div class="flex-1 overflow-y-auto py-1">
      ...
      {:else if filteredProjects.length === 0}
        <p class="text-xs text-text-muted px-3 py-4 text-center">Không có dự án nào có bộ nhớ</p>
      {:else}
  ...
  <div class="flex-1 overflow-y-auto p-6">
    {#if !selectedProject}
      <div class="flex flex-col items-center justify-center h-full text-text-muted">
        ...
        <p class="text-sm">Chọn một dự án để duyệt bộ nhớ</p>
        <p class="text-xs mt-1">Chỉ hiển thị các dự án có file bộ nhớ</p>
      </div>
    {:else}
      <!-- Header -->
      ...
      {#if memoryFiles.length === 0 && !showCreate}
        <div class="text-center py-12 text-sm text-text-muted">
          Không có file bộ nhớ nào trong dự án này
        </div>
      {:else}
    ...
    <div class="w-[450px] shrink-0 border-l border-border flex flex-col bg-bg-secondary">
      ...
      <div class="flex items-center justify-between px-4 py-3 border-b border-border shrink-0">
        <span class="text-sm font-medium text-text-primary truncate">
          {showCreate ? "Bộ nhớ mới" : editingFile?.filename}
        </span>
        <div class="flex items-center gap-2">
          {#if saveMessage}
          ...
          {/if}
          {#if editingFile && !showCreate}
            <button class="p-1 text-text-muted hover:text-danger" onclick={() => (deleteDialogOpen = true)} aria-label="Xóa">
              <Trash2 size={14} />
            </button>
          {/if}
          <button
          ...
          >{saving ? "..." : "Lưu"}</button>
          <button class="p-1 text-text-muted hover:text-text-primary" onclick={() => { editingFile = null; showCreate = false; }} aria-label="Đóng">
            <X size={16} />
          </button>
        </div>
      </div>
      ...
      <div class="px-4 py-3 border-b border-border space-y-2">
        ...
        <div class="grid grid-cols-2 gap-2">
          ...
          <label class="block">
            <span class="text-xs text-text-muted">Loại</span>
        ...
        <label class="block">
          <span class="text-xs text-text-muted">Mô tả</span>
          <input type="text" class="w-full mt-1 px-2 py-1 text-sm bg-bg-tertiary border border-border rounded text-text-primary focus:outline-none focus:border-accent" placeholder="Mô tả một dòng..." bind:value={editDescription} />
        </label>
      </div>
      ...
      <div class="px-4 pt-3 pb-1">
        <div class="flex gap-1 bg-bg-tertiary rounded-lg p-1 w-fit">
          <button class="px-3 py-1 text-xs rounded-md {!previewMode ? 'bg-bg-secondary text-text-primary' : 'text-text-muted'}" onclick={() => (previewMode = false)}>Chỉnh sửa</button>
          <button class="px-3 py-1 text-xs rounded-md {previewMode ? 'bg-bg-secondary text-text-primary' : 'text-text-muted'}" onclick={() => (previewMode = true)}>Xem trước</button>
        </div>
      </div>
