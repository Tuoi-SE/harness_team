<script lang="ts">
  ...
  async function loadRules() {
    ...
    } catch (e) { console.error("Lỗi:", e); rules = []; }
...
<ConfirmDialog
  ...
  title="Xóa quy tắc"
  message="Quy tắc này sẽ bị xoá vĩnh viễn."
  ...
  <div class="w-64 shrink-0 border-r border-border flex flex-col bg-bg-secondary">
    <div class="p-3 border-b border-border space-y-2">
      <div class="flex gap-1 bg-bg-tertiary rounded-lg p-1">
        <button class="flex-1 px-3 py-1.5 text-xs rounded-md transition-colors {scope === 'global' ? 'bg-bg-secondary text-text-primary' : 'text-text-muted'}" onclick={() => { scope = "global"; selected = null; editing = false; loadRules(); }}>Toàn cục</button>
        <button class="flex-1 px-3 py-1.5 text-xs rounded-md transition-colors {scope === 'project' ? 'bg-bg-secondary text-text-primary' : 'text-text-muted'}" onclick={() => { scope = "project"; selected = null; editing = false; loadRules(); }}>Dự án</button>
      </div>
      {#if needsProject}
      ...
      <div class="relative">
        ...
        <input type="text" class="w-full pl-8 pr-3 py-1.5 text-xs bg-bg-tertiary border border-border rounded-md text-text-primary placeholder:text-text-muted focus:outline-none focus:border-accent" placeholder="Tìm quy tắc..." bind:value={searchQuery} />
      </div>
    </div>

    <div class="flex-1 overflow-y-auto py-1">
      {#if loading}
        <p class="text-xs text-text-muted px-3 py-4 text-center">Đang tải...</p>
      {:else if needsProject && !projectPath}
        <p class="text-xs text-text-muted px-3 py-4 text-center">Chọn một dự án</p>
      {:else}
  ...
  <div class="flex-1 flex min-w-0">
    {#if editing || isNew}
      ...
      <div class="flex-1 flex flex-col">
        <div class="flex items-center justify-between px-6 py-3 border-b border-border shrink-0">
          <div class="flex items-center gap-3">
            {#if isNew}
              <label class="flex items-center gap-2">
                <span class="text-xs text-text-muted">Tên file</span>
          ...
          <div class="flex items-center gap-2">
            ...
            <div class="flex gap-1 bg-bg-tertiary rounded-lg p-0.5">
              <button class="px-2 py-1 text-[10px] rounded {!previewMode ? 'bg-bg-secondary text-text-primary' : 'text-text-muted'}" onclick={() => (previewMode = false)}>Chỉnh sửa</button>
              <button class="px-2 py-1 text-[10px] rounded {previewMode ? 'bg-bg-secondary text-text-primary' : 'text-text-muted'}" onclick={() => (previewMode = true)}>Xem trước</button>
            </div>
            <button class="px-4 py-1.5 text-sm bg-accent hover:bg-accent-hover text-white rounded-md disabled:opacity-50" onclick={save} disabled={saving || (isNew && !editFilename.trim())}>{saving ? "..." : "Lưu"}</button>
            <button class="p-1 text-text-muted hover:text-text-primary" onclick={() => { editing = false; isNew = false; }} aria-label="Đóng"><X size={16} /></button>
          </div>
        </div>
        ...
        <div class="px-6 py-3 border-b border-border">
          <label class="block">
            <span class="text-xs text-text-muted flex items-center gap-1"><Filter size={10} /> Bộ lọc đường dẫn (mỗi dòng một mục - quy tắc chỉ áp dụng cho file khớp)</span>
        ...
        <div class="flex-1 overflow-hidden p-4">
          ...
          {:else}
            <textarea class="w-full h-full px-4 py-3 text-sm bg-bg-secondary border border-border rounded-lg text-text-primary font-mono resize-none focus:outline-none focus:border-accent" placeholder="# Tiêu đề quy tắc&#10;&#10;- Quy tắc 1&#10;- Quy tắc 2" bind:value={editContent}></textarea>
          {/if}
        </div>
      </div>
    {:else if selected}
      ...
      <div class="flex-1 overflow-y-auto p-6 space-y-4">
        <div class="flex items-center justify-between">
          ...
          <div class="flex items-center gap-2">
            <button class="px-3 py-1.5 text-xs bg-bg-tertiary border border-border rounded-md text-text-secondary hover:border-accent/30" onclick={startEdit}>Chỉnh sửa</button>
            <button class="p-1.5 text-text-muted hover:text-danger rounded" onclick={() => (deleteDialogOpen = true)} aria-label="Xóa"><Trash2 size={14} /></button>
          </div>
        </div>
        ...
        {#if selected.paths_filter.length > 0}
          <div class="bg-bg-secondary border border-border rounded-lg p-4">
            <h3 class="text-xs font-medium text-text-muted uppercase tracking-wider mb-2 flex items-center gap-1.5">
              <Filter size={12} /> Bộ lọc đường dẫn
            </h3>
            <p class="text-xs text-text-muted mb-2">Quy tắc này chỉ áp dụng khi làm việc với các file khớp mẫu sau:</p>
        ...
        {:else}
          <div class="bg-bg-secondary border border-border rounded-lg p-3 flex items-center gap-2">
            ...
            <span class="text-xs text-text-muted">Không có bộ lọc đường dẫn - quy tắc này áp dụng ở mọi nơi</span>
          </div>
        {/if}
    ...
    {:else}
      <div class="flex-1 flex flex-col items-center justify-center text-text-muted">
        ...
        <p class="text-sm">Chọn một quy tắc hoặc tạo mới</p>
        <p class="text-xs mt-1">Quy tắc định hướng hành vi của Claude cho các mẫu file cụ thể</p>
      </div>
    {/if}
  </div>
</div>
