<script lang="ts">
  ...
  const NODE_PALETTE = [
    { type: "input", label: "Bắt đầu", icon: Play, color: "text-success" },
    { type: "claude", label: "Prompt Claude", icon: Bot, color: "text-accent" },
    { type: "bash", label: "Lệnh Bash", icon: Terminal, color: "text-success" },
    { type: "git", label: "Git", icon: GitCommitHorizontal, color: "text-info" },
    { type: "github", label: "GitHub CLI", icon: GitBranch, color: "text-info" },
    { type: "http", label: "Yêu cầu HTTP", icon: Globe, color: "text-danger" },
    { type: "filter", label: "Bộ lọc (If)", icon: Filter, color: "text-accent" },
    { type: "transform", label: "Biến đổi", icon: Repeat, color: "text-warning" },
    { type: "jsonextract", label: "Trích JSON", icon: Braces, color: "text-accent" },
    { type: "readfile", label: "Đọc file", icon: FileInput, color: "text-info" },
    { type: "writefile", label: "Ghi file", icon: FileOutput, color: "text-warning" },
    { type: "notification", label: "Thông báo", icon: Bell, color: "text-warning" },
    { type: "delay", label: "Trì hoãn", icon: Clock, color: "text-text-muted" },
    { type: "output", label: "Kết thúc", icon: CircleDot, color: "text-text-muted" },
  ];
...
<div class="relative w-full h-full" role="region" aria-label="Khung pipeline" oncontextmenu={onContextMenu}>
  <SvelteFlow
    {nodeTypes}
    bind:nodes
    bind:edges
  ...
  {#if showPalette}
    <button class="fixed inset-0 z-40" onclick={() => (showPalette = false)} aria-label="Đóng"></button>
    <div
    ...
    >
      <p class="text-[10px] text-text-muted px-2 pb-1 uppercase tracking-wider">Thêm node</p>
  ...
  <!-- Edge context menu -->
  {#if edgeMenu}
    <button class="fixed inset-0 z-40" onclick={() => (edgeMenu = null)} aria-label="Đóng"></button>
    <div
