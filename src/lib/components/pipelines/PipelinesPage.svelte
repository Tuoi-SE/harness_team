<script lang="ts">
  ...
  let activePipelineName = $state("Pipeline mới");
  ...
  function createNew() {
    ...
    activePipelineName = "Pipeline mới";
    flowNodes = [
      { id: crypto.randomUUID(), type: "input", position: { x: 50, y: 200 }, data: { label: "Bắt đầu", config: {}, status: "idle" } },
      { id: crypto.randomUUID(), type: "output", position: { x: 600, y: 200 }, data: { label: "Kết thúc", config: {}, status: "idle" } },
    ];
    flowEdges = [];
    clearResults();
    selectedNode = null;
  ...
  <div class="flex items-center justify-between px-4 py-2 border-b border-border bg-bg-secondary shrink-0">
    <div class="flex items-center gap-2">
      ...
      <div class="relative">
        ...
        {#if showPipelineList}
          <button class="fixed inset-0 z-40" onclick={() => (showPipelineList = false)} aria-label="Đóng"></button>
          <div class="absolute top-full left-0 mt-1 w-64 bg-bg-secondary border border-border rounded-lg shadow-xl z-50 max-h-48 overflow-y-auto">
            {#each pipelines as pipeline}
              ...
              >
                ...
                <span class="text-[10px] text-text-muted">{pipeline.nodes.length} node</span>
              </button>
            {/each}
            {#if pipelines.length === 0}
              <p class="px-3 py-2 text-xs text-text-muted">Chưa có pipeline nào</p>
            {/if}
          </div>
        {/if}
      </div>
    ...
    {#if activePipelineId}
      <div class="flex items-center gap-2">
        {#if saveMessage}
          <span class="text-xs {saveMessage.startsWith('Lỗi') ? 'text-danger' : 'text-success'}">{saveMessage}</span>
        {/if}
        {#if running}
          <button
        ...
        <div class="relative">
          <button
            ...
            title={scheduleEnabled ? `Đã lên lịch: ${scheduleExpr}` : "Lên lịch pipeline"}
          >
            <CalendarClock size={12} />
            {scheduleEnabled ? "Đã lên lịch" : "Lên lịch"}
          </button>

          {#if showSchedulePopover}
            <button class="fixed inset-0 z-40" onclick={() => (showSchedulePopover = false)} aria-label="Đóng"></button>
            <div class="absolute top-full right-0 mt-1 w-72 bg-bg-secondary border border-border rounded-lg shadow-xl z-50 p-3 space-y-3">
              <div class="flex items-center justify-between">
                <span class="text-xs font-medium text-text-primary">Lịch pipeline</span>
                {#if scheduleEnabled}
                  <span class="text-[10px] px-1.5 py-0.5 bg-success/20 text-success rounded">Đang hoạt động</span>
                {/if}
              </div>
              ...
              <!-- Custom cron input -->
              <div>
                <label class="text-[10px] text-text-muted">Biểu thức cron</label>
                <input
                  type="text"
                ...
                />
                <p class="text-[9px] text-text-muted mt-0.5">phút giờ ngày tháng thứ</p>
              </div>
        ...
        {#if showRightPanel}
          <div class="w-96 shrink-0 border-l border-border bg-bg-secondary flex flex-col">
            ...
            <div class="flex border-b border-border shrink-0">
              ...
              >
                Nhật ký {scheduleLogs.length > 0 ? `(${scheduleLogs.length})` : ""}
              </button>
              <button
              ...
              >
                Lịch sử {historyRuns.length > 0 ? `(${historyRuns.length})` : ""}
              </button>
            </div>
            ...
            {#if rightTab === "config" && selectedNode && selectedNode.type !== "input" && selectedNode.type !== "output"}
              <div class="flex-1 overflow-y-auto p-4 space-y-4">
                ...
                {#if nodeType === "claude"}
                  <div class="space-y-3">
                    <div>
                      <label class="text-xs text-text-muted font-medium">Lời nhắc</label>
                      <CodeEditor
                        value={nodeConfig.prompt ?? ""}
                        language="text"
                        placeholder="Claude nên làm gì?"
                        minHeight="120px"
                        maxHeight="400px"
                    ...
                    <label class="flex items-center gap-2">
                      ...
                      />
                      <span class="text-xs text-text-muted">Chế độ tương tác</span>
                    </label>
                    <p class="text-[10px] text-text-muted -mt-2 ml-5">Chạy Claude trong terminal — bạn có thể cấp quyền và trả lời câu hỏi. Khi tắt, dùng chế độ headless <code class="text-accent">--print</code>.</p>
                    ...
                    <label class="flex items-center gap-2">
                      ...
                      />
                      <span class="text-xs text-text-muted">Tự động chấp nhận mọi quyền</span>
                    </label>
                    <p class="text-[10px] text-text-muted -mt-2 ml-5">Thêm <code class="text-accent">--dangerously-skip-permissions</code>. Claude sẽ không hỏi xác nhận. Chỉ dùng trong môi trường an toàn.</p>

                    <label class="block">
                      <span class="text-xs text-text-muted font-medium">Thư mục làm việc</span>
                      <input
                        type="text"
                        ...
                        placeholder="~/projects/my-app (mặc định: home)"
                ...
                {:else if nodeType === "bash" || nodeType === "github"}
                  <div class="space-y-2">
                    <label class="text-xs text-text-muted font-medium">{nodeType === "bash" ? "Lệnh" : "Lệnh GitHub CLI"}</label>
                    <CodeEditor
                    ...
                    />
                    <p class="text-[10px] text-text-muted">Dùng {"{{"}input{"}}"} cho output trước đó hoặc {"{{"}NodeName{"}}"} cho node cụ thể</p>
                  </div>
                ...
                {:else if nodeType === "http"}
                  <div class="space-y-4">
                    ...
                    <div class="grid grid-cols-4 gap-2">
                      <label class="block">
                        <span class="text-xs text-text-muted font-medium">Phương thức</span>
                        <select
                    ...
                    {#if showBody}
                      <div class="space-y-2">
                        <label class="text-xs text-text-muted font-medium">Nội dung</label>
                        <CodeEditor
                    ...
                    <p class="text-[10px] text-text-muted">Dùng {"{{"}input{"}}"} hoặc {"{{"}NodeName{"}}"} trong URL, headers hoặc nội dung.</p>
                  </div>
              ...
              <div class="flex-1 overflow-y-auto flex flex-col">
                ...
                {:else if historyRuns.length === 0}
                  <div class="flex flex-col items-center justify-center h-full text-text-muted p-4">
                    ...
                    <p class="text-xs">Chưa có lịch sử chạy</p>
                    <p class="text-[10px] mt-1">Hãy chạy pipeline này để xem lịch sử</p>
                  </div>
                {:else}
                  <div class="flex items-center justify-between px-3 py-2 border-b border-border shrink-0">
