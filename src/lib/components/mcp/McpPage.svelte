...
<ConfirmDialog
  ...
  title="Xóa server"
  message="Server '{deletingServerName}' sẽ bị xoá vĩnh viễn."
...
<div class="flex h-full">
  ...
  <div class="flex-1 overflow-y-auto p-6 space-y-6">
    ...
    {#if needsProject && !projectPath}
      <div class="flex items-center justify-center h-48 text-sm text-text-muted">Chọn một dự án</div>
    {:else if loading}
      ...
      {#if cloudMcps.length > 0}
        <div>
          <h3 class="text-xs font-medium text-text-muted uppercase tracking-wider mb-2 flex items-center gap-1.5">
            <Cloud size={12} />
          Cloud (được cấu hình tại claude.ai)
          </h3>
            ...
            {#each cloudMcps as name}
              <div class="bg-bg-secondary border border-border rounded-lg p-3 flex items-center gap-3 opacity-70">
                ...
                <div>
                  ...
                  <p class="text-xs text-text-muted">Được quản lý qua claude.ai</p>
                </div>
              </div>
            {/each}
          </div>
      ...
      <div>
        ...
        {:else}
          <div class="bg-bg-secondary border border-border rounded-lg p-8 text-center">
            ...
            <p class="text-sm text-text-muted mb-1">Chưa có MCP server nào được cấu hình</p>
            <p class="text-xs text-text-muted">Thêm server hoặc duyệt mẫu</p>
          </div>
        {/if}
      </div>
  ...
  {#if editing}
    <div class="w-[400px] shrink-0 border-l border-border flex flex-col bg-bg-secondary">
      <div class="flex items-center justify-between px-4 py-3 border-b border-border">
        <span class="text-sm font-medium text-text-primary">{isNew ? "Thêm server" : `Sửa: ${editing.name}`}</span>
        <button class="p-1 text-text-muted hover:text-text-primary" onclick={() => (editing = null)} aria-label="Đóng">
          <X size={16} />
        </button>
      </div>
      ...
      <div class="flex-1 overflow-y-auto p-4 space-y-4">
        <label class="block">
          <span class="text-xs text-text-muted">Tên server</span>
        ...
        <div>
          ...
          <div class="flex gap-1 mt-1" role="group" aria-label="Server type">
            {#each [{ id: "stdio" as const, label: "Lệnh (stdio)", icon: Terminal }, { id: "sse" as const, label: "URL (SSE/HTTP)", icon: Globe }] as t}
              <button
        ...
        {#if formType === "stdio"}
          <label class="block">
            <span class="text-xs text-text-muted">Lệnh</span>
          ...
          <label class="block">
            <span class="text-xs text-text-muted">Tham số</span>
            ...
            <p class="text-[10px] text-text-muted mt-1">Tham số cách nhau bằng dấu cách</p>
          </label>
        {:else}
          <label class="block">
      ...
      <div class="px-4 py-3 border-t border-border flex justify-end gap-2">
        <button class="px-4 py-1.5 text-sm text-text-muted hover:text-text-secondary" onclick={() => (editing = null)}>Hủy</button>
        <button
