<script lang="ts">
  ...
  async function exportSession() {
    ...
    } catch (e) { console.error("Lỗi xuất:", e); }
  }
  ...
  <div class="w-80 shrink-0 border-r border-border flex flex-col bg-bg-secondary">
    <div class="p-3 border-b border-border">
      <div class="relative">
        ...
        <input type="text" class="w-full pl-8 pr-3 py-1.5 text-xs bg-bg-tertiary border border-border rounded-md text-text-primary placeholder:text-text-muted focus:outline-none focus:border-accent" placeholder="Tìm tất cả phiên..." bind:value={searchQuery} onkeydown={(e) => e.key === "Enter" && doSearch()} />
        {#if searchQuery.trim()}
    ...
    <div class="flex-1 overflow-y-auto">
      ...
      {#if liveSessions.length > 0 && !searchMode}
        <div class="px-3 py-2 border-b border-border">
          <p class="text-[10px] text-success uppercase tracking-wider flex items-center gap-1 mb-1">
            <Radio size={10} class="animate-pulse" /> Trực tiếp ({liveSessions.length})
          </p>
      ...
      {#if searchMode}
        <div class="px-3 py-2 border-b border-border flex items-center justify-between">
          <span class="text-xs text-text-muted">{searching ? "Đang tìm..." : `${searchResults.length} kết quả`}</span>
          <button class="text-xs text-accent" onclick={() => { searchMode = false; searchQuery = ""; }}>Xóa</button>
        </div>
        ...
        {/each}
      {:else if loading}
        <p class="text-xs text-text-muted text-center py-4">Đang tải phiên...</p>
      {:else if filteredSessions.length === 0}
        <p class="text-xs text-text-muted text-center py-4">Không tìm thấy phiên nào</p>
      {:else}
        ...
        {#if hasMore}
          <div class="p-3">
            ...
            >
              {loadingMore ? "Đang tải..." : `Tải thêm (${sessions.length}/${totalCount})`}
            </button>
          </div>
        {/if}
      {/if}
  ...
  <div class="flex-1 flex flex-col min-w-0">
    {#if !selectedSession}
      <div class="flex-1 flex flex-col items-center justify-center text-text-muted">
        ...
        <p class="text-sm">Chọn một phiên để phát lại</p>
        <p class="text-xs mt-1">Duyệt các phiên Claude Code cũ từng bước</p>
      </div>
    {:else if loadingEvents}
      <div class="flex-1 flex items-center justify-center text-sm text-text-muted">Đang tải phiên...</div>
    {:else}
      <!-- Session header -->
      <div class="flex items-center justify-between px-6 py-3 border-b border-border shrink-0 bg-bg-secondary">
        ...
        <div class="flex items-center gap-3">
          ...
          <button
            ...
            title="Xuất Markdown và sao chép vào clipboard"
          >
            {#if copiedExport}
      ...
      <div class="flex-1 overflow-y-auto px-6 py-4 space-y-3">
          ...
          {#if event.type === "user"}
            ...
            <div class="flex gap-3">
              ...
              <div class="flex-1 min-w-0">
                <p class="text-[10px] text-text-muted mb-1">Bạn · {formatTime(event.timestamp)}</p>
                <div class="bg-info/5 border border-info/20 rounded-lg px-4 py-2">
                  {#if userText.length > 300 && !expandedEvents.has(idx)}
                    ...
                    <button class="text-xs text-accent mt-1" onclick={() => toggleExpand(idx)}>
                      Xem thêm ({userText.length} ký tự)
                    </button>
                  {:else}
            ...
            {#if resultText}
              <div class="ml-10">
                ...
                >
                  ...
                  {/if}
                  Output công cụ ({resultText.length} ký tự)
                </button>
                {#if expandedEvents.has(idx)}
        ...
        {#if hasMoreEvents && !playing}
          <div class="py-4">
            ...
            >
              {loadingMoreEvents ? "Đang tải..." : `Tải thêm sự kiện (${events.length}/${totalEvents})`}
            </button>
          </div>
        {/if}
      </div>
