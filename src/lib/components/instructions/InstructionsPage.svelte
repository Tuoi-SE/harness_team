...
<div class="p-6 overflow-y-auto h-full flex flex-col gap-4">
  <div class="flex items-center justify-between">
    <div class="flex items-center gap-3">
      <div class="flex gap-1 bg-bg-tertiary rounded-lg p-1">
        {#each [
          { id: "global" as const, label: "Toàn cục" },
          { id: "project" as const, label: "Dự án" },
          { id: "project-dot" as const, label: "Dự án (.claude/)" },
          { id: "local" as const, label: "Cục bộ (bị git bỏ qua)" },
        ] as tab}
          <button
  ...
  {#if needsProject && !projectPath}
    <div class="flex items-center justify-center h-48 text-sm text-text-muted">
      Chọn một dự án để chỉnh sửa hướng dẫn
    </div>
  {:else if loading}
  ...
  {:else}
    ...
    {#if file?.imports && file.imports.length > 0}
      <div class="bg-bg-secondary border border-border rounded-lg p-3">
        <p class="text-xs text-text-muted mb-2">Các import được tham chiếu:</p>
    ...
    {#if !file?.exists && !previewMode}
      <div class="bg-bg-secondary border border-border rounded-lg p-4 text-center">
        <p class="text-sm text-text-muted mb-2">File này chưa tồn tại</p>
        <p class="text-xs text-text-muted">Bắt đầu gõ bên dưới để tạo file</p>
      </div>
    {/if}
    ...
    {:else}
      <textarea
        ...
        placeholder="# Hướng dẫn cho Claude Code..."
        bind:value={content}
      ></textarea>
    {/if}
  {/if}
