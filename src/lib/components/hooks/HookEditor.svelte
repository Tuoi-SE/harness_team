...
<div class="space-y-4">
  ...
  {#if !configs || configs.length === 0}
    <div class="bg-bg-secondary border border-border rounded-lg p-6 space-y-4">
      <p class="text-sm text-text-muted text-center">Chưa có hook nào cho sự kiện này</p>

      <!-- Quick templates -->
      <div class="grid grid-cols-2 gap-2">
        {#each [
          { label: "Lệnh shell", desc: "Chạy script trước hoặc sau", matcher: "", type: "command" as const, command: "echo 'hook triggered'" },
          { label: "HTTP webhook", desc: "Gửi tới một endpoint", matcher: "", type: "http" as const, url: "http://localhost:8080/hook" },
          { label: "Prompt guard", desc: "AI kiểm tra hành động", matcher: "", type: "prompt" as const, prompt: "Validate this action is safe" },
          { label: "Bash validator", desc: "Kiểm tra lệnh bash", matcher: "Bash", type: "command" as const, command: ".claude/hooks/validate.sh" },
        ] as tpl}
          <button
      ...
      <div class="bg-bg-secondary border border-border rounded-lg p-4 space-y-3">
        <div class="flex items-center justify-between">
          <div class="flex items-center gap-2">
            ...
            <span class="text-xs text-text-muted">Bộ khớp:</span>
            <input
              type="text"
              aria-label="Mẫu matcher"
