<script lang="ts">
  ...
  async function loadHooks() {
    ...
    } catch (e) { console.error("Lỗi tải hooks:", e); rawHooks = {}; }
  ...
  function addFromForm() {
    ...
    else handler.prompt = newValue || "Kiểm tra hành động này";
...
<ConfirmDialog
  ...
  title="Xóa hook"
  message="Hook này sẽ bị xoá. Hãy lưu để áp dụng thay đổi."
...
<div class="flex flex-col h-full">
  ...
  <div class="flex items-center justify-between px-6 py-3 border-b border-border shrink-0">
    <div class="flex items-center gap-3">
      <div class="flex gap-1 bg-bg-tertiary rounded-lg p-1">
        {#each [{ id: "global" as const, label: "Toàn cục" }, { id: "project" as const, label: "Dự án" }] as tab}
          <button
  ...
  {#if needsProject && !projectPath}
    <div class="flex items-center justify-center flex-1 text-sm text-text-muted">Chọn một dự án</div>
  {:else if loading}
    <div class="flex items-center justify-center flex-1 text-sm text-text-muted">Đang tải...</div>
  {:else}
      ...
      <div class="flex-1 overflow-y-auto p-6 space-y-4">
        {#if !selectedEvent}
          <div class="flex flex-col items-center justify-center h-full text-text-muted">
            ...
            <p class="text-sm">Chọn một sự kiện để cấu hình hook</p>
            <p class="text-xs mt-1">Hook sẽ chạy tự động khi sự kiện xảy ra trong Claude Code</p>
          </div>
        {:else}
          <div>
          ...
          {#if showAddForm}
            <div class="bg-bg-secondary border border-accent/30 rounded-lg p-4 space-y-3">
              <div class="grid grid-cols-2 gap-3">
                <label class="block">
                  <span class="text-xs text-text-muted">Bộ khớp <span class="text-text-muted">(vd: Bash, Edit, * cho tất cả)</span></span>
                ...
                <div>
                  <span class="text-xs text-text-muted">Kiểu</span>
              ...
              <label class="block">
                <span class="text-xs text-text-muted">{newType === "command" ? "Lệnh" : newType === "http" ? "URL" : "Prompt"}</span>
                <input type="text" class="w-full mt-1 px-3 py-1.5 text-sm bg-bg-tertiary border border-border rounded-md text-text-primary font-mono focus:outline-none focus:border-accent" placeholder={newType === "command" ? "/path/to/script.sh" : newType === "http" ? "https://..." : "Mô tả điều cần kiểm tra..."} bind:value={newValue} />
              </label>
              <div class="flex justify-end gap-2">
                <button class="px-4 py-1.5 text-sm text-text-muted hover:text-text-secondary" onclick={() => (showAddForm = false)}>Hủy</button>
                <button class="px-4 py-1.5 text-sm bg-accent hover:bg-accent-hover text-white rounded-md" onclick={addFromForm}>Thêm</button>
              </div>
            </div>
          {:else}
            <button
...
<TemplateGallery
  ...
  onselect={(template) => {
    ...
    else if (hookType === "prompt") handler.prompt = template.hookValue ?? "Kiểm tra hành động này";
