<script lang="ts">
  ...
  async function installPlugin(fullName: string) {
    ...
    try {
      ...
      actionMessage = `Đã cài ${fullName.split("@")[0]}!`;
  ...
  function formatDate(dateStr: string): string {
    if (!dateStr) return "";
    return new Date(dateStr).toLocaleDateString("vi", { month: "short", day: "numeric", year: "numeric" });
  }
...
<ConfirmDialog
  ...
  title="Gỡ plugin"
  message="'{uninstallingPlugin}' sẽ bị gỡ."
  confirmLabel="Gỡ"
...
<div class="p-6 overflow-y-auto h-full space-y-4">
  ...
  <div class="relative">
    ...
    <input type="text" class="w-full pl-9 pr-3 py-2 text-sm bg-bg-secondary border border-border rounded-md text-text-primary placeholder:text-text-muted focus:outline-none focus:border-accent" placeholder="Tìm plugin trong {activeTab === 'installed' ? 'danh sách đã cài' : 'chợ plugin'}..." bind:value={search} />
  </div>
  ...
  {:else if activeTab === "installed"}
          ...
          <div class="bg-bg-secondary border border-border rounded-lg p-4 group">
            ...
            <div class="flex items-center gap-3 mt-3 text-xs text-text-muted">
              ...
              {#if isBlocked(plugin.name)}
                <span class="px-1.5 py-0.5 rounded bg-danger/10 text-danger">bị chặn</span>
              {/if}
            </div>
          </div>
        {/each}
    ...
    {:else}
      <div class="bg-bg-secondary border border-border rounded-lg p-12 text-center">
        ...
        <p class="text-sm text-text-muted mb-1">{search ? "Không có plugin nào khớp" : "Chưa có plugin nào được cài"}</p>
        <p class="text-xs text-text-muted">Duyệt chợ plugin để khám phá plugin mới</p>
      </div>
    {/if}
  ...
  {:else}
          ...
          <div class="bg-bg-secondary border border-border rounded-lg p-4 flex flex-col justify-between group hover:border-accent/30 transition-colors">
            <div>
              ...
              <p class="text-xs text-text-muted mt-2">
                ...
                {formatInstalls(plugin.unique_installs)} lượt cài
              </p>
            </div>
            <div class="mt-3">
              {#if alreadyInstalled}
                <span class="text-xs text-success">Đã cài</span>
              {:else}
                <button
                ...
                >
                  {installing === plugin.plugin ? "Đang cài..." : "Cài đặt"}
                </button>
              {/if}
            </div>
          </div>
    ...
    {:else}
      <div class="bg-bg-secondary border border-border rounded-lg p-12 text-center">
        ...
        <p class="text-sm text-text-muted">{search ? "Không có plugin nào khớp với tìm kiếm" : "Chưa có dữ liệu chợ plugin"}</p>
      </div>
    {/if}
  {/if}
</div>
