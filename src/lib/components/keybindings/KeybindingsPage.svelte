<script lang="ts">
  ...
  async function load() {
    ...
    try {
      ...
      console.error("Lỗi tải keybindings:", e);
      bindings = [];
    } finally {
      loading = false;
    }
...
<div class="p-6 overflow-y-auto h-full flex flex-col gap-4">
  ...
  <div class="flex items-center justify-between">
    ...
    <div class="flex items-center gap-3">
      {#if saveMessage}
        <span class="text-xs {saveMessage.startsWith('Lỗi') ? 'text-danger' : 'text-success'}">
          {saveMessage}
        </span>
      {/if}
      <button
  ...
  {#if loading}
    <p class="text-sm text-text-muted">Đang tải keybindings...</p>
  {:else if bindings.length === 0}
    <div class="flex flex-col items-center justify-center h-48 gap-3">
      <Keyboard size={32} class="text-text-muted" />
      <p class="text-sm text-text-muted">Chưa cấu hình keybinding nào</p>
      <button
  ...
  {:else}
      ...
      <table class="w-full">
        <thead>
          <tr class="border-b border-border text-xs text-text-muted uppercase tracking-wider">
            <th class="text-left px-4 py-3 w-48">Phím</th>
            <th class="text-left px-4 py-3 w-48">Lệnh</th>
            <th class="text-left px-4 py-3">Mô tả</th>
            <th class="text-left px-4 py-3 w-48">Khi nào</th>
            ...
            <tr class="border-b border-border last:border-b-0 hover:bg-bg-hover/50 transition-colors">
              ...
              <td class="px-4 py-2">
                <input
                  ...
                  oninput={markChanged}
                  placeholder="Chức năng của phím tắt này"
    ...
    <div class="bg-bg-secondary border border-border rounded-lg p-4">
      <p class="text-xs font-medium text-text-secondary mb-2">Tham chiếu định dạng phím</p>
      <div class="grid grid-cols-2 gap-x-8 gap-y-1 text-xs text-text-muted">
        <span><code class="text-accent">Ctrl+K</code> — phím bổ trợ + phím</span>
        <span><code class="text-accent">Escape Escape</code> — tổ hợp tuần tự</span>
        <span><code class="text-accent">Shift+Tab</code> — có Shift</span>
        <span><code class="text-accent">Enter</code> — một phím đơn</span>
        <span><code class="text-accent">Ctrl+Shift+P</code> — nhiều phím bổ trợ</span>
        <span><code class="text-accent">Up</code> / <code class="text-accent">Down</code> — phím mũi tên</span>
      </div>
    </div>
  {/if}
</div>
