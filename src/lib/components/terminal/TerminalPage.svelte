...
<div class="flex flex-col h-full">
  ...
  <div class="flex items-center justify-between px-4 py-3 border-b border-border bg-bg-secondary shrink-0">
    <div class="flex items-center gap-3">
      ...
      >
        {starting ? "Đang khởi động..." : "+ Phiên mới"}
      </button>
    </div>
    {#if error}
  ...
  <div class="flex-1 relative bg-[#0d1117]">
    {#if sessions.length === 0}
      <div class="absolute inset-0 flex flex-col items-center justify-center text-text-muted">
        ...
        <p class="text-sm mb-1">{projectPath ? "Bấm + Phiên mới để bắt đầu" : "Hãy chọn dự án trước"}</p>
        <p class="text-xs text-text-muted">Các phiên vẫn được giữ khi bạn chuyển sang màn hình khác</p>
      </div>
    {/if}
    <div
