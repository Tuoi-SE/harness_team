<script lang="ts">
  ...
  const actions: PaletteAction[] = [
    ...
    },
    {
      id: "action-theme",
      label: "Đổi giao diện",
      description: "Chuyển giữa chế độ sáng và tối",
      icon: "theme",
      category: "action",
      handler: toggleTheme,
    },
        ...
        {#each filtered as action, i}
          ...
          >
            ...
            <span class="text-[10px] px-1.5 py-0.5 rounded bg-bg-tertiary text-text-muted uppercase">
              {action.category === "navigate" ? "trang" : "hành động"}
            </span>
          </button>
        {/each}
      {/if}
