<script lang="ts">
  ...
  const costLabel = $derived(isApiPlan ? "Chi phí ước tính" : "Tương đương API");
...
<div class="p-6 overflow-y-auto h-full space-y-4">
  {#if loading}
    <p class="text-sm text-text-muted">Đang tải phân tích...</p>
  {:else if !stats}
    <p class="text-sm text-text-muted">Chưa có dữ liệu phân tích</p>
  {:else}
    <!-- Top Stats -->
    <div class="grid grid-cols-2 lg:grid-cols-4 gap-3">
      {#each [
        { label: costLabel, value: formatCost(totalCost()), sub: `${formatCost(costPerMessage())}/tin nhắn${!isApiPlan ? " · Gói " + planType.charAt(0).toUpperCase() + planType.slice(1) : ""}`, icon: DollarSign, color: "text-accent" },
        { label: "Tổng token", value: formatTokens(totalTokens()), sub: `trung bình ${formatTokens(avgDailyTokens())}/ngày`, icon: Coins, color: "text-info" },
        { label: "Phiên dài nhất", value: stats.longestSession ? formatDuration(stats.longestSession.duration) : "—", sub: stats.longestSession ? `${formatNumber(stats.longestSession.messageCount)} tin nhắn` : "", icon: Clock, color: "text-warning" },
        { label: "Hoạt động từ", value: stats.firstSessionDate ? new Date(stats.firstSessionDate).toLocaleDateString("vi", { month: "short", year: "numeric" }) : "—", sub: stats.firstSessionDate ? `${Math.floor((Date.now() - new Date(stats.firstSessionDate).getTime()) / 86400000)} ngày trước` : "", icon: Calendar, color: "text-success" },
      ] as card}
    ...
    <div class="bg-bg-secondary border border-border rounded-lg p-4">
      <div class="flex items-center gap-2 mb-4">
        ...
        <h3 class="text-sm font-medium text-text-secondary">Mức dùng model</h3>
      </div>
      <div class="space-y-5">
        {#each modelEntries() as entry}
          ...
          <div>
            ...
            <div class="grid grid-cols-4 gap-3">
              {#each [
                { label: "Đầu vào", value: entry.inputTokens, color: "bg-info" },
                { label: "Đầu ra", value: entry.outputTokens, color: "bg-success" },
                { label: "Đọc cache", value: entry.cacheReadInputTokens, color: "bg-warning" },
                { label: "Ghi cache", value: entry.cacheCreationInputTokens, color: "bg-accent" },
              ] as tok}
                <div>
      ...
      <div class="bg-bg-secondary border border-border rounded-lg p-4">
        <div class="flex items-center justify-between mb-3">
          <div class="flex items-center gap-2">
            ...
            <h3 class="text-sm font-medium text-text-secondary">Token theo ngày (30 ngày)</h3>
          </div>
          <span class="text-xs text-text-muted">trung bình {formatTokens(avgDailyTokens())}/ngày</span>
        </div>
        <div class="flex items-end gap-[2px] h-36">
          {#each dailyTokenTrend() as day}
            ...
            <div
              ...
              onmouseenter={(e) => showTooltip(e, new Date(day.date).toLocaleDateString("vi", { month: "short", day: "numeric" }), formatTokens(day.total) + " token")}
      ...
      <div class="bg-bg-secondary border border-border rounded-lg p-4">
        <div class="flex items-center justify-between mb-3">
          <div class="flex items-center gap-2">
            ...
            <h3 class="text-sm font-medium text-text-secondary">Hoạt động theo giờ</h3>
          </div>
          {#if peakHour()}
            <span class="text-xs text-text-muted">Đỉnh: {peakHour()!.hour}:00 ({peakHour()!.count} phiên)</span>
          {/if}
        </div>
        <div class="flex items-end gap-[2px] h-36">
    ...
    <div class="bg-bg-secondary border border-border rounded-lg p-4">
      <div class="flex items-center gap-2 mb-4">
        ...
        <h3 class="text-sm font-medium text-text-secondary">Hiệu quả cache</h3>
      </div>

      <div class="grid grid-cols-3 gap-6">
        <div>
          <p class="text-xs text-text-muted mb-1">Tỷ lệ trúng cache</p>
          <p class="text-2xl font-bold text-success">{cacheRatio.toFixed(1)}%</p>
          <p class="text-[10px] text-text-muted mt-1">{formatTokens(cacheTotalRead)} token từ cache</p>
        </div>
        <div>
          <p class="text-xs text-text-muted mb-1">Tổng lượng cache</p>
          <p class="text-2xl font-bold text-info">{formatTokens(cacheTotalRead + cacheTotalWrite)}</p>
          <p class="text-[10px] text-text-muted mt-1">{formatTokens(cacheTotalRead)} lượt đọc · {formatTokens(cacheTotalWrite)} lượt ghi</p>
        </div>
        <div>
          <p class="text-xs text-text-muted mb-1">Mức tiết kiệm ước tính</p>
          <p class="text-2xl font-bold text-success">{formatCost(cacheSavedCost)}</p>
          <p class="text-[10px] text-text-muted mt-1">so với token đầu vào tính đủ giá</p>
        </div>
      </div>
      ...
      <div class="mt-4">
        <div class="h-4 bg-bg-tertiary rounded-full overflow-hidden flex">
          <div
            ...
            role="button" tabindex="-1"
            onmouseenter={(e) => showTooltip(e, "Đầu vào trực tiếp", formatTokens(cacheTotalInput))}
            onmouseleave={() => (chartTooltip = null)}
          ></div>
          <div
            ...
            role="button" tabindex="-1"
            onmouseenter={(e) => showTooltip(e, "Lượt đọc cache", formatTokens(cacheTotalRead))}
            onmouseleave={() => (chartTooltip = null)}
          ></div>
          <div
            ...
            role="button" tabindex="-1"
            onmouseenter={(e) => showTooltip(e, "Lượt ghi cache", formatTokens(cacheTotalWrite))}
            onmouseleave={() => (chartTooltip = null)}
          ></div>
        </div>
        <div class="flex gap-4 mt-2 text-xs text-text-muted">
          <span class="flex items-center gap-1.5"><span class="w-2.5 h-2.5 rounded-sm bg-info"></span> Đầu vào ({((cacheTotalInput / cacheTotal) * 100).toFixed(1)}%)</span>
          <span class="flex items-center gap-1.5"><span class="w-2.5 h-2.5 rounded-sm bg-success"></span> Đọc cache ({((cacheTotalRead / cacheTotal) * 100).toFixed(1)}%)</span>
          <span class="flex items-center gap-1.5"><span class="w-2.5 h-2.5 rounded-sm bg-accent"></span> Ghi cache ({((cacheTotalWrite / cacheTotal) * 100).toFixed(1)}%)</span>
        </div>
      </div>
    </div>
  {/if}
