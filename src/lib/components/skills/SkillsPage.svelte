<script lang="ts">
  ...
  async function loadItems() {
    ...
    } catch (e) { console.error("Lỗi:", e); items = []; }
  ...
  function startCreate() {
    ...
    const template = type === "skill"
      ? `---\nname: my-${type}\ndescription: ${type} này làm gì\nuser-invocable: true\n---\n\n# Hướng dẫn\n\nMô tả Claude nên làm gì khi ${type} này được gọi.\n`
      : `---\nname: my-${type}\ndescription: Khi nào nên giao cho agent này\nmodel: sonnet\ntools: Read, Glob, Grep\n---\n\n# System Prompt\n\nBạn là một trợ lý chuyên biệt...\n`;
...
<ConfirmDialog
  ...
  title="Xóa {isAgent ? 'agent' : 'skill'}"
  message="{isAgent ? 'Agent' : 'Skill'} này và toàn bộ file liên quan sẽ bị xoá vĩnh viễn."
...
<div class="flex h-full">
  ...
  <div class="w-64 shrink-0 border-r border-border flex flex-col bg-bg-secondary">
    ...
    <div class="p-3 border-b border-border space-y-2">
      ...
      <div class="flex gap-1 bg-bg-tertiary rounded-lg p-1">
        <button class="flex-1 px-2 py-1 text-[10px] rounded transition-colors {scope === 'global' ? 'bg-bg-secondary text-text-primary' : 'text-text-muted'}" onclick={() => { scope = "global"; loadItems(); }}>Toàn cục</button>
        <button class="flex-1 px-2 py-1 text-[10px] rounded transition-colors {scope === 'project' ? 'bg-bg-secondary text-text-primary' : 'text-text-muted'}" onclick={() => { scope = "project"; loadItems(); }}>Dự án</button>
      </div>
      {#if needsProject}
      ...
      <div class="relative">
        ...
        <input type="text" class="w-full pl-8 pr-3 py-1.5 text-xs bg-bg-tertiary border border-border rounded-md text-text-primary placeholder:text-text-muted focus:outline-none focus:border-accent" placeholder="Tìm..." bind:value={searchQuery} />
      </div>
    </div>
  ...
  <div class="flex-1 flex min-w-0">
      ...
      <div class="flex-1 flex flex-col">
        <div class="flex items-center justify-between px-6 py-3 border-b border-border shrink-0">
          <div class="flex items-center gap-3">
            {#if isNew}
              <label class="block">
                <span class="text-xs text-text-muted">Tên</span>
          ...
          <div class="flex items-center gap-2">
            ...
            <button class="px-4 py-1.5 text-sm bg-accent hover:bg-accent-hover text-white rounded-md disabled:opacity-50" onclick={save} disabled={saving || (isNew && !editName.trim())}>{saving ? "..." : "Lưu"}</button>
            <button class="p-1 text-text-muted hover:text-text-primary" onclick={() => { editing = false; isNew = false; }} aria-label="Đóng">
              <X size={16} />
            </button>
          </div>
        </div>
        ...
        {#if activeTab === "skills" && !previewMode}
          <div class="px-6 py-2 bg-bg-tertiary/50 border-b border-border text-[10px] text-text-muted flex gap-4">
            <span><code class="text-accent">$ARGUMENTS</code> = đầu vào người dùng</span>
            <span><code class="text-accent">$0</code>, <code class="text-accent">$1</code> = tham số vị trí</span>
            <span><code class="text-accent">!`cmd`</code> = chạy trước prompt</span>
            <span><code class="text-accent">${"${CLAUDE_SKILL_DIR}"}</code> = thư mục skill</span>
          </div>
        {/if}
      ...
      <div class="flex-1 overflow-y-auto p-6 space-y-4">
        ...
        <div class="flex items-center justify-between">
          ...
          <div class="flex items-center gap-2">
            ...
            <button class="p-1.5 text-text-muted hover:text-danger rounded" onclick={() => (deleteDialogOpen = true)} aria-label="Xóa">
              <Trash2 size={14} />
            </button>
          </div>
        </div>
        ...
        <div class="grid grid-cols-2 lg:grid-cols-3 gap-3">
            ...
            <div class="bg-bg-secondary border border-border rounded-lg p-3">
              ...
              <p class="text-sm text-text-secondary">
                {#if selectedParsed.meta["disable-model-invocation"]}
                  Chỉ thủ công (/<span class="text-accent">{selected.name}</span>)
                {:else if selectedParsed.meta["user-invocable"] === false}
                  Chỉ tự động (Claude tự gọi)
                {:else}
          ...
          {#if selectedParsed.meta.mcp}
            <div class="bg-bg-secondary border border-border rounded-lg p-3">
              <div class="flex items-center gap-2 text-xs text-text-muted mb-1">
                <Server size={12} />
                <span>MCP nội tuyến</span>
              </div>
              <p class="text-sm text-accent">Đã cấu hình</p>
            </div>
          {/if}
        </div>
        ...
        {#if selectedParsed.meta.skills || selectedParsed.meta.mcp || selectedParsed.meta.memory || selectedParsed.meta.hooks}
          <div class="bg-bg-secondary border border-border rounded-lg p-4">
            <h3 class="text-xs font-medium text-text-muted uppercase tracking-wider mb-3">Liên kết</h3>
        ...
        <!-- Content preview -->
        <div class="bg-bg-secondary border border-border rounded-lg p-4">
          <h3 class="text-xs font-medium text-text-muted uppercase tracking-wider mb-3">Hướng dẫn</h3>
    ...
    {:else}
      ...
      <div class="flex-1 flex flex-col items-center justify-center text-text-muted">
        ...
        {/if}
        <p class="text-sm">Chọn một {isAgent ? "agent" : "skill"} hoặc tạo mới</p>
        <p class="text-xs mt-1">
          {isAgent
            ? "Agents là trợ lý AI chuyên biệt với ngữ cảnh tách biệt"
            : "Skills mở rộng khả năng của Claude bằng các lệnh tuỳ biến"}
        </p>
      </div>
    {/if}
  </div>
