...
<div class="p-6 space-y-6 overflow-y-auto h-full">
  {#if loading}
    <div class="flex items-center justify-center h-64">
      <p class="text-text-muted">Đang tải...</p>
    </div>
  {:else}
    <!-- Stats Row -->
    <StatsOverview
        ...
        {#if costSummary}
          <div class="space-y-3">
            ...
            <div class="flex items-center justify-between text-xs">
              <span class="text-text-muted flex items-center gap-1"><TrendingUp size={10} /> Dự đoán</span>
            ...
            <p class="text-[10px] text-text-muted/60 leading-tight">Dựa trên đơn giá token API, không phải cước thuê bao</p>
          </div>
        {:else}
          <p class="text-xs text-text-muted">Chưa có dữ liệu chi phí</p>
        {/if}
      </div>
      ...
      <div class="bg-bg-secondary border border-border rounded-lg p-4">
        ...
        {#if last7Days.length > 0}
            ...
            {#each last7Days as day, i}
              ...
              <div class="flex-1 flex flex-col items-center gap-0.5 group relative">
                ...
                <span class="text-[9px] text-text-muted">
                  {dayDate.toLocaleDateString("vi", { weekday: "narrow" })}
                </span>
          ...
          <p class="text-xs text-text-muted mt-2 text-right">
            Tổng: <span class="text-text-secondary font-medium">${weekTotal.toFixed(2)}</span>
          </p>
        {:else}
          <p class="text-xs text-text-muted">Chưa có dữ liệu</p>
        {/if}
      </div>
    </div>
      ...
      <div class="bg-bg-secondary border border-border rounded-lg p-4">
              ...
              <div class="space-y-1">
                <div class="flex items-center justify-between text-xs">
                  ...
                  <span class="text-text-secondary font-medium shrink-0">
                    ...
                    <span class="text-text-muted font-normal ml-1">{formatNumber(project.messages)} tin nhắn</span>
                  </span>
                </div>
        ...
        {:else}
          <div class="flex flex-col items-center justify-center py-6 text-text-muted">
            ...
            <p class="text-xs">Chưa có dữ liệu chi phí dự án</p>
          </div>
        {/if}
      </div>
    </div>
