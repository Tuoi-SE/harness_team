<script lang="ts">
  ...
  const items = $derived<HealthItem[]>([
    {
      label: "MCP Server",
      value: mcpCount > 0 ? `${mcpCount} đã kết nối` : "Không có",
      ok: mcpCount > 0,
      icon: Server,
      page: "mcp",
    },
    {
      label: "Hooks",
      value: hookCount > 0 ? `${hookCount} đang hoạt động` : "Không có",
      ok: hookCount > 0,
      icon: Zap,
      page: "hooks",
    },
    {
      ...
      value: hasPermissions ? "Đã cấu hình" : "Chưa đặt",
      ok: hasPermissions,
      icon: Shield,
      page: "settings",
    },
...
<div class="bg-bg-secondary border border-border rounded-lg p-4">
  <h3 class="text-sm font-medium text-text-secondary mb-3">Tình trạng cấu hình</h3>
  {#if loading}
