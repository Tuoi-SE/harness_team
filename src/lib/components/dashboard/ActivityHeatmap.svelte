...
<div class="bg-bg-secondary border border-border rounded-lg p-4">
  <h3 class="text-sm font-medium text-text-secondary mb-3">Hoạt động</h3>
  ...
  <div class="flex items-center gap-2 mt-3 text-xs text-text-muted">
    ...
    <span>Nhiều</span>
    <span class="ml-auto">{dailyActivity.filter((d) => d.messageCount > 0).length} ngày đã theo dõi</span>
  </div>
</div>
...
{#if tooltip}
  ...
  >
    <p class="text-text-primary font-medium">
      {new Date(tooltip.date).toLocaleDateString("vi", { weekday: "short", month: "short", day: "numeric", year: "numeric" })}
    </p>
    <p class="text-text-secondary">
      {formatNumber(tooltip.count)} tin nhắn · {tooltip.sessions} phiên · {formatNumber(tooltip.tools)} công cụ
    </p>
  </div>
{/if}
